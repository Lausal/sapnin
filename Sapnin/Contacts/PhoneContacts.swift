...
class PhoneContacts{
    ...
    class func getContacts() -> [CNContact] {
        ...
        do {
//            var p = NSPredicate.init(format: "", argumentArray: nil)
        ...
        return removeAllNumberLessContacts(contacts: results)
    }
    
    
    class func removeAllNumberLessContacts(contacts : [CNContact]) -> [CNContact]{
        
        var tempContacts = [CNContact]()
        for c in contacts{
            if c.phoneNumbers.count >= 1{
                tempContacts.append(c)
            }
        }
